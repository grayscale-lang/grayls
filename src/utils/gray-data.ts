...
export const DOCS: Record<string, string> = {
  ...
  // --- C interop ---
  'c': '**`c`** \u2014 The reserved C interop module.\n\nImport a C header, then call into it through the `c.` prefix:\n\n```gray\nimport c"stdio.h"           // system header \u2192 #include <stdio.h>\nimport c"./mylib.h"         // local header  \u2192 #include "./mylib.h"\nimport c"stdio.h", c"stdlib.h"\n\ndo main() {\n    c.puts("hello from C")\n    c.printf("value: %d\\n", 42)\n    println(c.EOF)          // constants and macros too\n}\n```\n\nGrayscale strings convert to `char*` automatically. Convert a C `char*` back with `c_string()`:\n\n```gray\nmut home string = c_string(c.getenv("HOME"))\n```\n\n**Cannot be passed to C:** `i128`, `i256`, `u128`, `u256`, arrays, maps, and Grayscale structs.\n\nThe module name `c` is reserved \u2014 a local `c.gray` must be imported under an alias.\n\n\u26a0\ufe0f C interop will be redesigned before 1.0.',

  // --- Wildcard types ---
  '?': '**`?`** \u2014 Wildcard type. A placeholder that enables generic-style functions.\n\n```gray\ndo identity(x ?) -> ? {\n    return x\n}\n\nmut a = identity(42)        // ? binds to int\nmut b = identity("hello")   // ? binds to string\n```\n\nAll `?` placeholders in one signature bind to the **same** concrete type, inferred from the first `?` argument:\n\n```gray\ndo pick_first(a ?, b ?) -> ? { return a }\n\npick_first(1, 2)         // OK\npick_first(1, "hello")   // Error: conflicting bindings for ?\n```\n\nWorks with composite types: `do first(arr [?]) -> ?`.\n\n**`?` is only valid in function parameter and return types.** It is rejected in variable declarations, struct fields, `new(?)`, and named return positions. A `?` return requires at least one `?` parameter to provide the binding.',
  '<?>': '**`<?>`** \u2014 Type parameter. Lets a function parameter accept a struct **type name** rather than a value.\n\n```gray\nconst Point struct {\n    x int\n    y int\n}\n\ndo make(T <?>) -> ^? {\n    return new(T)\n}\n\nmut p = make(Point)   // -> ^Point\n```\n\nInside the body, the type parameter is valid in `new(T)`, struct literals `T{x: 1}`, and `size_of(T)`.\n\nThe return type uses `?`: `-> ^?` resolves to a pointer to the type argument, `-> ?` to the type argument itself.\n\n**Restrictions:**\n- Type and value parameters cannot be mixed in one signature (E2087).\n- Only struct type names may be passed \u2014 enums, primitives, and expressions are rejected (E3127, E3128).',
